#!/usr/bin/env python3
'''
BLASTS a sequence against online database and returns the top hit.
'''
...
def get_args():
    ...
    parser = argparse.ArgumentParser(description='Script for performing web BLAST Search',
                                     usage="get_mito_genome.py -q QUERY -d DATABASE \
                                         -b BLAST_TYPE [OPTIONS]",
...
def do_blast(seq, blast_type, database):
    '''
    Performs BLAST search.

    :param seq: query sequence
    :type seq: str
    :param blast_type: blastn, blastp, blastx, tblastn, or tblastx
    :type blast_type: str
    :param database: wev database to be used
    :type database: str
    :return: blast record
    :rtype: Bio.Blast.Record.Blast
    '''
    blast_results = NCBIWWW.qblast(program=blast_type, database=database, sequence=seq)
...
if __name__ == '__main__':
    ...
    with mp.Pool(processes=int(args.threads)) as p:
        # Iterable to be used in p.map.
        # This iterates through each sequence in the input fasta file.
        _iter = [record.format(format='fasta') for record in records]

        # first argument is function to be run in parallel.
        # 2nd and 3rd arguments are function arguments
        # that are the same for all iterations of the function
        func = partial(do_blast, blast_type=args.blast_type, database=args.database)
